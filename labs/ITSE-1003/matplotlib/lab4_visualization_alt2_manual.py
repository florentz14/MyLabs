# ------------------------------------------------------------ #
# File: lab4_visualization_alt2_manual.py
# Date: 2026-04-26
# Author: Florentino Baez
# Professor: Mauricio Quiroga
# Description: Lab 4 — Alternative 2: count categories manually with a Python loop (no value_counts(), no .to_numpy()).
# Explanation: Same charts as the original lab, but in Part 6 the category counts are built with a basic for loop and Python lists.
# ------------------------------------------------------------ #
...
plt.scatter(
    ...
    edgecolor="black" # color of the scatter points
)
...
# ----------------------------------------------------------------------------
# Part 6: Pie Chart (Category distribution)  -- ALTERNATIVE 2
# Porpose: Show the distribution of categories.
# Alternative: Build the counts manually with a for loop and plain Python lists.
# Only basic features are used: list, loop, comparison and .sum().
# ----------------------------------------------------------------------------

# Define the categories we want to count (must match the values in the CSV).
categories = ["A", "B", "C"]

# Build the counts list manually with a for loop.
counts = []
for cat in categories:
    count = (df["Category"] == cat).sum()
    counts.append(count)

# Optional sanity-check: print the counts.
print("\nCategory counts:")
...
# Plot the category distribution using plain Python lists.
plt.pie(
    counts,
    labels=categories,
    autopct="%1.1f%%",
...
# Right subplot: Category pie chart (reuse the lists from Part 6).
plt.subplot(1, 2, 2)

# Plot the category distribution (reuse counts/categories/pie_colors from Part 6).
plt.pie(
    counts,
    labels=categories,
    autopct="%1.1f%%",
